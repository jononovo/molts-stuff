      ...
      <div className="max-w-5xl mx-auto text-[12px] flex justify-between items-center">
        ...
        <div className="text-[11px]">
          {isLoading ? (
            ...
            <span>
              <span className="text-gray-600">{user?.firstName || user?.email || 'user'}</span>
              {user?.linkedAgentId && (
                <Link href={`/u/${user.firstName || 'me'}`} className="text-[#0000cc] hover:underline ml-2">
                  [profile]
                </Link>
              )}
              <button 
                onClick={() => logout()}
                className="text-[#0000cc] hover:underline ml-2 bg-transparent border-none cursor-pointer"
